...
function RelationCountry( rect, d, c ) {	
	...
	////////////this.bg = new Shape();
	this.txt = new createjs.Text( this.name, "bold 10px Arial", "#FFFFFF" );	
	var w =  this.txt.getMeasuredWidth();
	this.txt.x = (rect.width/2) - (w/2);
	this.txt.y = rect.height/2;
	this.txt.alpha = .6;
	
	this.amountTxt = new createjs.Text( Math.ceil(this.data.percent*100), "bold 10px Arial", "#FFFFFF" );
	if( this.rect.width>30 ) this.amountTxt.text += "%";	
	
	this.amountTxt.x = this.txt.x + ((w-this.amountTxt.getMeasuredWidth())/2);
	this.amountTxt.y = (rect.height/2) + 10;
	this.amountTxt.alpha = .6;
	...
	image.onload = function(){
		...
		if( image<0 || diffX<0 || diffY<0 || newW<0 || newH<0 || wDest<0 || hDest<0 ){
			
		}
		
		
		//console.log( image+", "+diffX+", "+diffY+", "+newW+", "+newH+", "+wDest+", "+hDest );
  ...
  {
  console.log(wDest+", "+hDest+", "+err);
  ctx.drawImage(image,0,0,image.width, image.height,0,0,wDest,hDest);
  }
		
				
		var bitmap = new createjs.Bitmap(dummyCanvas);
...
RelationCountry.prototype = new createjs.Container();
